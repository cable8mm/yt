            ...
            <div class="row video-show" style="background-color:black">
                <div class="col-lg-12 col-sm-12 col-xs-12">
                    <div class="embed-responsive embed-responsive-16by9 video-player">
                        {!! preg_replace(
                            '/(src="[^"]+)"/',
                            "\\1" . '?autoplay=1&cc_load_policy=1&rel=0&hd=1&hl=en&cc_lang_pref=ko"',
                            $video->embed_html,
                        ) !!}
                ...
                @foreach ($channelVideos as $video)
                    <div class="media">
                        ...
                        <div class="media-body">
                            <h4 class="media-heading channel-video-title"><a
                                    href="/videos/{{ $video->id }}">{{ $video->title }}</a></h4>
                            <p class="info"><span class="label label-info">Published</span><span
                                    class="info-date">{{ date(
                                        "Y년
                                                                m월 d일",
                                        strtotime($video->published_at),
                                    ) }}</span>
                            </p>
                        </div>
                    </div>
